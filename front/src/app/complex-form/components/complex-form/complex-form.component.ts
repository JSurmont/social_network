...
export class ComplexFormComponent implements OnInit {
  ...
  showEmailFormCtrl$!: Observable<boolean>;
  showPhoneFormCtrl$!: Observable<boolean>;
  ...
  initFormObservables() {
    this.showEmailFormCtrl$ = this.contactPreferenceCtrl.valueChanges.pipe(
      startWith(this.contactPreferenceCtrl.value),
      map((preference: string) => preference === 'email'),
      tap(showEmailCtrl => this.setEmailValidators(showEmailCtrl))
    );
    this.showPhoneFormCtrl$ = this.contactPreferenceCtrl.valueChanges.pipe(
      startWith(this.contactPreferenceCtrl.value),
  ...
  private setEmailValidators(showEmailCtrl: boolean) {
    if (showEmailCtrl) {
      ...
      this.confirmEmailCtrl.clearAsyncValidators();
    }
  ...
  onSubmitForm() {
    this.loading = true;
    this.complexFormService.saveUsernfo(this.mainForm.value).pipe(
      tap(saved => {
        this.loading = false;
        if (saved) {
          this.resetForm();
